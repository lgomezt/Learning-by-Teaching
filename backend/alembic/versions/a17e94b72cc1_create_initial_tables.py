...
def upgrade() -> None:
    """Upgrade schema."""
    # Create enum type for MessageType
    op.execute("CREATE TYPE messagetype AS ENUM ('CHAT', 'CODE', 'OUTPUT')")
    ...
    sa.Column('update_log', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    sa.PrimaryKeyConstraint('problem_id')
    )
    op.create_index(op.f('ix_problems_problem_id'), 'problems', ['problem_id'], unique=False)
    op.create_unique_constraint('uq_problems_file_path', 'problems', ['file_path'])
...
def downgrade() -> None:
    ...
    op.drop_constraint('uq_problems_file_path', 'problems', type_='unique')
